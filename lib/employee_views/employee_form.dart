...
class EmployeeFormPage extends StatefulWidget {
  const EmployeeFormPage({Key? key, required this.dashBoardArgs})
      : super(key: key);
  final Map? dashBoardArgs;
  @override
...
class _EmployeeFormPageState extends State<EmployeeFormPage>
  ...
  XFile? pickedFile;
  late bool tabCountTwo;
  @override
  void initState() {
    super.initState();
    tabCountTwo = widget.dashBoardArgs != null &&
        widget.dashBoardArgs?['tabCountTwo'] == true;
    int tabcount = tabCountTwo ? 2 : 1;
    _tabController = TabController(length: tabcount, vsync: this);
    _tabController.addListener(() {
      _tabController.addListener(() {
        if (!_tabController.indexIsChanging) {
          if (tabcount != 2 && _tabController.index == 1) {
  ...
  //   final args = widget.dashBoardArgs ??
  ...
  Future<void> getEmployeeDetails() async {
    ...
    if (token == null || typedServerUrl == null || empId.isEmpty) return;

    final uri = Uri.parse('$typedServerUrl/api/employee/employees/$empId');
    ...
    if (response.statusCode == 200) {
      ...
      // });
      await Future.wait([
        getEmployeeWorkInformation(withSetState: false),
        getEmployeeBankInformation(withSetState: false),
        _loadEmployeeData(),
      ]);
  ...
  //     // ✅ Async calls done outside setState
  //     await getEmployeeWorkInformation();
  //     await getEmployeeBankInformation();
  //     await _loadEmployeeData(); // Load SharedPrefs & set employeeId

  //     if (!mounted) return;

  //     // ✅ Now safely call controller init
  //     initializeController(); // Use employeeId here
  ...
  Future<void> getWorkTypeRequest() async {
    final args = (ModalRoute.of(context)?.settings.arguments is Map)
        ? ModalRoute.of(context)!.settings.arguments as Map
        : widget.dashBoardArgs ?? {};
    //final args = ModalRoute.of(context)!.settings.arguments as Map??widget.dashBoardArgs;
    empId = args['employee_id'].toString();
    final prefs = await SharedPreferences.getInstance();
    var token = prefs.getString("token");
    var typedServerUrl = prefs.getString("typed_url");
  ...
  //   }
  // }
  Future<void> getEmployeeWorkInformation({bool withSetState = true}) async {
    final prefs = await SharedPreferences.getInstance();
    var token = prefs.getString("token");
    var typedServerUrl = prefs.getString("typed_url");
    employeeWorkInfoId = employeeDetails['employee_work_info_id'];
    ...
    if (response.statusCode == 200) {
      ...
      if (withSetState) {
        setState(() {
          employeeWorkInfoRecord = decoded;
        });
      } else {
        employeeWorkInfoRecord = decoded;
      }
      getEmployeeJobRole(); // if this is slow, also move it out
    }
  }
  ...
  //   }
  // }
  Future<void> getEmployeeBankInformation({bool withSetState = true}) async {
    final prefs = await SharedPreferences.getInstance();
    var token = prefs.getString("token");
    var typedServerUrl = prefs.getString("typed_url");
    employeeBankDetailsId = employeeDetails['employee_bank_details_id'] ?? '';
...
''';
                                  ...
                                  child: TabBarView(
                                    ...
                                    children: [
                                      Shimmer.fromColors(
                                        baseColor: Colors.grey[300]!,
                                        highlightColor: Colors.grey[100]!,
                                        child: buildTabContentAboutTest(
                                          context,
                                          employeeDetails,
                                          employeeWorkInfoRecord,
                                          employeeBankRecord,
  ...
  aboutTabChange(index) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (index == 0) {
        ...
        _workScrollController.jumpTo(0);
      } else {
        _bankScrollController.jumpTo(0);
      }
    });
  }
  ...
  Widget buildTabContentAboutTest(
    ...
    List tags = employeeWorkInfoRecord['tags'] ?? [];
    ...
    tagsController.text = tagTitles.join(', ');
    return Column(
      children: [
        Row(
          spacing: 10,
          children: [
            InkWell(
              ...
              child: Container(
                ...
                height: 26,
                decoration: BoxDecoration(
                  gradient: aboutIndex == 0
                      ? const LinearGradient(
                          colors: [
                            Color(0xFF1E8ED1),
                            Color(0xFF054A91), // Dark Blue
                            // Light Blue
                          ],
                      ...
                      : const LinearGradient(
                          colors: [
                            Colors.grey, // Light grey start
                            Colors.grey, // Light grey end
                          ],
                        ),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  spacer(),
                  aboutHeader('Date of Birth'),
                  aboutValue(dateOfBirthController.text),
                  spacer(),
                  aboutHeader('Gender'),
                  aboutValue(genderController.text),
                  spacer(),
                  aboutHeader('Address'),
                  aboutValue(addressController.text),
                  spacer(),
                  aboutHeader('Country'),
                  aboutValue(countryController.text),
                  spacer(),
                  aboutHeader('State'),
                  aboutValue(stateController.text),
                  spacer(),
                  aboutHeader('City'),
                  aboutValue(cityController.text),
                  spacer(),
                  aboutHeader('Qualification'),
                  aboutValue(qualificationController.text),
                  spacer(),
                  aboutHeader('Experience'),
                  aboutValue(experienceController.text),
                  spacer(),
                  aboutHeader('Marital Status'),
                  aboutValue(maritalStatusController.text),
                  spacer(),
                  aboutHeader('Children'),
                  aboutValue(childrenController.text),
                  spacer(),
                  aboutHeader('Emargency Contact'),
                  aboutValue(emergencyContactController.text),
                  spacer(),
                  aboutHeader('Contact Name'),
                  aboutValue(emergencyContactNameController.text),
                  spacer(),
                  spacer(),
                  spacer(),
                ],
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  spacer(),
                  aboutHeader('Department'),
                  aboutValue(departmentController.text),
                  spacer(),
                  aboutHeader('Job Position'),
                  aboutValue(jobPositionController.text),
                  spacer(),
                  aboutHeader('Shift Information'),
                  aboutValue(shiftController.text),
                  spacer(),
                  aboutHeader('Work Type'),
                  aboutValue(workTypeController.text),
                  spacer(),
                  aboutHeader('Employee Type'),
                  aboutValue(employeeTypeController.text),
                  spacer(),
                  aboutHeader('Salary'),
                  aboutValue(salaryController.text),
                  spacer(),
                  aboutHeader('Reporting Manager'),
                  aboutValue(reportingManagerController.text),
                  spacer(),
                  aboutHeader('Company'),
                  aboutValue(companyController.text),
                  spacer(),
                  aboutHeader('Work Location'),
                  aboutValue(locationController.text),
                  spacer(),
                  aboutHeader('Joining Date'),
                  aboutValue(joiningDateController.text),
                  spacer(),
                  aboutHeader('End Date'),
                  aboutValue(endDateController.text),
                  spacer(),
                  aboutHeader('Tags'),
                  aboutValue(tagsController.text),
                  spacer(val: 30.0),
                ],
              ),
            ),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  spacer(),
                  aboutHeader('Bank Name'),
                  aboutValue(bankNameController.text),
                  spacer(),
                  aboutHeader('Bank Address'),
                  aboutValue(bankAddressController.text),
                  spacer(),
                  aboutHeader('Account Number'),
                  aboutValue(accountNumberController.text),
                  spacer(),
                  aboutHeader('Country'),
                  aboutValue(countryController.text),
                  spacer(),
                  aboutHeader('Branch'),
                  aboutValue(branchController.text),
                  spacer(),
                  aboutHeader('State'),
                  aboutValue(stateController.text),
                  spacer(val: 30.0),
